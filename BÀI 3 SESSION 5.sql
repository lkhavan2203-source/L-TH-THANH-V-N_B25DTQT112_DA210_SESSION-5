...
-- Liệt kê đơn hàng có tổng giá trị > 1,000,000
SELECT *
FROM DonHang
WHERE TongGiaTri > 1000000;
-- Liệt kê đơn hàng được giao đến 'TP. Hồ Chí Minh'
SELECT *
FROM DonHang
WHERE ThanhPhoGiaoHang = 'TP. Hồ Chí Minh';
-- Liệt kê và sắp xếp đơn hàng theo TongGiaTri giảm dần
SELECT *
FROM DonHang
ORDER BY TongGiaTri DESC;
-- Liệt kê danh sách các thành phố (duy nhất) đã có đơn hàng
